...
class PatchRecorderSpec extends FlatSpec with Matchers with Checkers with MockFactory with WorldSpecSupport {
  ...
  def recordingActionFactoriesGenerator(seed: Long,
    ...
    def lifecyclesForAnIdGenerator(id: FooHistory#Id): Gen[LifecyclesForAnId] = inSequence {
      def lifecycleForAnIdGenerator(id: FooHistory#Id): Gen[LifecycleForAnId] = inAnyOrder {
        def patchesOfTheSameKindForAnIdGenerator(patchGenerator: FooHistory#Id => Gen[AbstractPatch]): Gen[PatchesOfTheSameKindForAnId] = for {
        ...
        for {
          ...
          Gen.sequence[Seq[PatchesOfTheSameKindForAnId], PatchesOfTheSameKindForAnId](Seq(patchGenerator(fooProperty1) _, patchGenerator(fooProperty2) _) map (patchesOfTheSameKindForAnIdGenerator(_)))
        } yield {
          val randomBehaviour = new Random(seed)
      ...
      val unconstrainedGenerator = for {
        recordingsItemFactoriesForFiniteLifecycles <- Gen.listOf(finiteLifecycleForAnIdGenerator(id))
    ...
    for {
      ...
      recordingActionFactoriesOverSeveralIds <- Gen.sequence[Seq[LifecyclesForAnId], LifecyclesForAnId](ids.toSeq map (lifecyclesForAnIdGenerator(_)))
    } yield {
      val randomBehaviour = new Random(seed)
