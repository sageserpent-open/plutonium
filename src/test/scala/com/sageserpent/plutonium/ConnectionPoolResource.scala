...
import scalikejdbc.{ConnectionPool, DBSession, DataSourceConnectionPool}
  ...
  def connectionPoolResource: Resource[IO, ConnectionPool] =
    for {
      ...
      dataSource <- Resource.make(IO {
        ...
        result.setJdbcUrl(
          s"jdbc:h2:file:${databaseDirectory.resolve(databaseName)};DB_CLOSE_ON_EXIT=FALSE;MV_STORE=0;ANALYZE_AUTO=5000;ANALYZE_SAMPLE=50000")
