...
trait WorldSpecSupportAmericium extends Assertions {
  ...
  def seedTrials: Trials[Long] = api.longs

  def instantTrials: Trials[Instant] = api.longs.map(Instant.ofEpochMilli)
  ...
  def integerDataSamplesForAnIdTrials =
    dataSamplesForAnIdTrials[IntegerHistory](
      ...
      Seq(integerHistoryDataSampleTrials(faulty = false))
    )
  ...
  def referenceToItemDataSamplesForAnIdTrials =
    dataSamplesForAnIdTrials[ReferringHistory](
      ...
      Seq(pertainingToAnotherItemDataSampleTrials(faulty = false))
    )
  ...
  def shuffleRecordingsPreservingRelativeOrderOfEventsAtTheSameWhen(
    ...
    if (shuffledEventsPerItemTrials.isEmpty) {
  ...
  def mixedRecordingsGroupedByIdTrials(
    ...
    val leftHandDataSamplesForAnIdTrials = api.alternateWithWeights(
      ...
      1 -> dataSamplesForAnIdTrials[MoreSpecificFooHistory](
        ...
        Seq(moreSpecificFooHistoryDataSampleTrials(faulty))
      )
      )
    )
    ...
    val rightHandDataSamplesForAnIdTrials = api.alternateWithWeights(
      ...
      1 -> dataSamplesForAnIdTrials[IntegerHistory](
        integerHistoryIdTrials,
        Seq(integerHistoryDataSampleTrials(faulty))
      )
      )
    )
  ...
  def recordingsGroupedByIdTrials_(
    ...
    api.integers(1, 3).flatMap(recordingsForAnIdTrials.listsOfSize).filter(idsAreNotRepeated)
  }
