...
trait StatefulItemProxyFactory extends ProxyFactory {
  ...
  // HACK: By default, ByteBuddy places the declaring type resolver early in the
  // chain of resolvers, which can lead
  // to either insane resolutions or pointless ambiguities. Move it down to the
  // end to workaround this when necessary.
    ...
    builder
      ...
      .intercept(
        MethodDelegation
          ...
          .withResolvers(
            saneAmbiguityResolver
          ) // HACK: otherwise 'recordAnnihilation' won't see its obvious counterpart
          // if it tries to bind against
          // 'StatefulItemProxyFactory.AcquiredState.recordAnnihilation'
  ...
  object mutation {
    ...
    def apply(
      ...
      if (
        acquiredState.itemIsLocked || acquiredState.invariantCheckInProgress
      ) {
  ...
  object checkInvariant {
    def apply(
        @FieldValue("acquiredState") acquiredState: AcquiredState,
        @SuperCall superCall: Callable[Unit]
    ): Unit = {
      apply(acquiredState)
