...
case class BlobStorageReferenceImplementation[Time, SnapshotBlob] private(
  ...
  override def openRevision(): RevisionBuilder = {
    class RevisionBuilderImplementation extends RevisionBuilder {
      ...
      override def build(): BlobStorage[Time, SnapshotBlob] = {
        ...
        val newEventRevisions: Map[Time, Revision] =
          thisBlobStorage.recordingRevisions ++ (recordings map {
            case (when, _) => when
        ...
        val newAndModifiedExplodedLifecycles =
          (Map
            ...
            case (explodedLifecycles, (when, snapshots)) =>
              val explodedLifecyclesWithDefault =
                explodedLifecycles withDefault {
                  case UniqueItemSpecification(id, itemClazz) =>
                    thisBlobStorage.lifecycles
                      .get(id)
                      .flatMap(
                        _.find(itemClazz == _.itemClazz)
                      ) getOrElse PhoenixLifecycleSpanningAnnihilations(
                      itemClazz = itemClazz
                    )
                }
  ...
  override def timeSlice(
    ...
    trait TimesliceImplementation extends Timeslice[SnapshotBlob] {
      override def uniqueItemQueriesFor[Item](
        ...
        lifecycles
          .flatMap { case (id, lifecyclesForThatId) =>
      ...
      override def snapshotBlobFor(
        ...
        for {
          ...
          snapshotBlob <- lifecycle.snapshotBlobFor(
            when,
  ...
  case class PhoenixLifecycleSpanningAnnihilations(
      ...
      snapshotBlobs: ScissOrderedSeq[
        ...
        Time
      ] = ScissOrderedSeq.empty(_._1, Ordering[Time].reverse)
  ) {
    ...
    def snapshotBlobFor(
      ...
      blobEntriesIterator
        .find { case (snapshotWhen, (_, blobRevision)) =>
          (inclusive || Ordering[Time].lt(
            snapshotWhen,
            when
          )) && blobRevision == validRevisionFor(snapshotWhen)
        }
        .collect { case (_, (Some(snapshotBlob), _)) =>
          snapshotBlob
        }
    }
    ...
    def retainUpTo(
        when: Time
    ): Option[PhoenixLifecycleSpanningAnnihilations] = {
