      ...
      .use(db =>
        IO {
          db localTx { implicit session: DBSession =>
            ...
            // that the starting value for 'LineageId' would follow on from
            // 'sentinelLineageId'. For now, we just have to be careful when
            // editing this.
            sql"""
              CREATE TABLE Lineage(
                LineageId       BIGINT  GENERATED BY DEFAULT AS IDENTITY (START WITH 0) PRIMARY KEY,
