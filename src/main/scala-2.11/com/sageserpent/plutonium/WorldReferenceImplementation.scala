...
class WorldReferenceImplementation extends World {
  ...
  class ScopeReferenceImplementation(val when: Unbounded[Instant], val asOf: Unbounded[Instant]) extends com.sageserpent.plutonium.Scope {
    ...
    def this(when: Unbounded[Instant], nextRevision: Revision) = this(when, nextRevision match {
      ...
      case nextRevisionAfterAnActualRevision => Finite(revisionTimeline(nextRevisionAfterAnActualRevision - 1))
    })
  ...
  override def scopeFor(when: Unbounded[Instant], nextRevision: Revision): Scope = new ScopeReferenceImplementation(when, nextRevision)

  // This produces a 'read-only' scope - raw objects that it renders from bitemporals will fail at runtime if an attempt is made to mutate them, subject to what the proxies can enforce.
  override def scopeFor(when: Unbounded[Instant], asOf: Instant): Scope = new ScopeReferenceImplementation(when, Finite(asOf))
}
