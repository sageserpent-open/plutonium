...
class Patch[Raw <: Identified : TypeTag](id: Raw#Id, method: Method, arguments: Array[AnyRef], methodProxy: MethodProxy) extends AbstractPatch(id, method) {
  ...
  def wrap(argument: AnyRef): WrappedArgument = argument match {
    case identified: Identified => \/-(identified.id, ???) // TODO - how do I get the type tag?
    case _ => -\/(argument)
  }

  val wrappedArguments = arguments map wrap

  def unwrap(identifiedItemAccess: IdentifiedItemAccess)(wrappedArgument: WrappedArgument) = wrappedArgument.fold(identity, {case(id, typeTag) => identifiedItemAccess.itemFor(id)})
